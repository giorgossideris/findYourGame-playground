...
public class GameDAO {
    ...
    try{
        ...
        while(rs.next()){
            ...
            list_of_games.add(new Game(rs.getInt("game_id"), rs.getInt("category_id"),
                                       rs.getString("gamename"),rs.getInt("start_age"),
                                       rs.getInt("end_age"), rs.getInt("min_players"), rs.getInt("max_players"),
                                       rs.getInt("duration"), rs.getDouble("rating_value"),
                                       rs.getString("game_description"), rs.getInt("number_of_ratings")));
        }
        rs.close();
        stmt.close();
        db.close();
    ...
    public List<Game> getTopRatedGames() throws Exception { //returns the top 5 best rated games
		...
		try {
			...
			while(rs.next()) {
				...
				topRatedGames.add(new Game(rs.getInt("game_id"), rs.getInt("category_id"),
                                           rs.getString("gamename"),rs.getInt("start_age"),
                                           rs.getInt("end_age"), rs.getInt("min_players"), rs.getInt("max_players"),
                                           rs.getInt("duration"), rs.getDouble("rating_value"),
                                           rs.getString("game_description"), rs.getInt("number_of_ratings")));
    ...
    public List<Game> getMostRatedGames() throws Exception { // returns the top 3 most rated games (with the most rating not top rated)
		...
		try {
			...
			while(rs.next()) {
				...
				mostRatedGames.add(new Game(rs.getInt("game_id"), rs.getInt("category_id"),
                                           rs.getString("gamename"),rs.getInt("start_age"),
                                           rs.getInt("end_age"), rs.getInt("min_players"), rs.getInt("max_players"),
                                           rs.getInt("duration"), rs.getDouble("rating_value"),
                                           rs.getString("game_description"), rs.getInt("number_of_ratings")));
